...
// Operations include:
// POST /files/create
// POST /files/:fileId/flatten
// GET /files/:fileId/contents
func Benchmark__All(b *testing.B) {
	logger := log.NewNopLogger()
	repo := NewRepositoryInMemory(24*time.Hour, logger)
	service := NewService(repo)
	// setup our HTTP handler
	...
	s := []struct {
		method            string
		url               string
		fileInRequestBody bool
	}{
		{
			method:            "POST",
			url:               fmt.Sprintf("%s/files/create", server.URL),
			fileInRequestBody: true,
		},
		{
			method:            "POST",
			url:               fmt.Sprintf("%s/files/%s/flatten", server.URL, f.ID),
			fileInRequestBody: false,
		},
		{
			method:            "GET",
			url:               fmt.Sprintf("%s/files/%s/contents", server.URL, f.ID),
			fileInRequestBody: false,
		},
	}
